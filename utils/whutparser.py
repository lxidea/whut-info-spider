...
class whutparser(object):
    ...
    def getCategory_and_facultyLink(self):
        ...
        self.catlist = [[addr.string.title(), addr.get("href")] for addr in
                        self.parser.soup.find("div", "in_nav").find_all("a") if
                        self.root in addr.get("href") and not addr.get("href").endswith(self.root)]
        facultyName = [namespan.find("h2").string.split()[0] for namespan in
                       self.parser.soup.find_all("div", "tit_box6")]
    ...
    def getNewsfromListPage(self, _url):
        ...
        NewsList = [[self.fullurl(li.find_all('a')[-1].get('href'), 1), li.find_all('a')[-1].get('title')] for li in
                    lilist]
    ...
    def getNewsListPages(self, name, root):
        ...
        for page in range(pageMax):
            if page != 0 and root.endswith('/'):
                url = root + 'index_' + str(page) + '.shtml'
            elif page != 0:
                url = root + '/index_' + str(page) + '.shtml'
            pages.append(url)
        return pages
...
if __name__ == '__main__':
    proxy = {'http': 'http://proxy.lxidea.org:26808'}
    myparser = whutparser()
    ...
    for li in myparser.catlist:
        print li[0].encode('utf8'), li[1].encode('utf8')
    news = myparser.getNewsContent(entries[0])
    print news.title.encode('utf8')
    pages = myparser.getNewsListPages(None,myparser.catlist[0][1])
    print pages
